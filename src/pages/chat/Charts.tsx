...
interface MultiSelectControl extends BaseControl {
    type: "multiselect";
    options: string[];
    defaultValue: string[];
}
...
const getChartDescription = (type: string): string => {
    switch (type) {
        case "feature_tree":
            return 'Этот график показывает "расстояние" между признаками. Если ветви сливаются ниже пунктирной линии (порога), эти параметры считаются дубликатами. Внутри одной группы (цвета) достаточно оставить только один признак, чтобы не перегружать модели машинного обучения. Чем меньше порог кластеров, тем больше сходство у найденых столбцов.';
        case "correlation":
            return "Тепловая карта показывает силу и направление линейной связи между числовыми параметрами. Значения, близкие к 1 или -1, указывают на сильную взаимосвязь, а близкие к 0 — на её отсутствие. Помогает быстро найти факторы, влияющие друг на друга.";
        case "trend_line":
            return "Отображает динамику изменения показателей во времени. Полезно для выявления трендов, сезонности и резких скачков (аномалий) в исторической перспективе.";
        case "pairplot":
            return 'Матрица рассеяния позволяет одним взглядом оценить попарные зависимости множества признаков, выявить кластеры и нелинейные паттерны, которые "не видит" обычная корреляция.';
        case "feature_importances":
            return "Показывает вес (важность) каждого параметра в предсказании целевой переменной. Чем длиннее полоса, тем сильнее этот признак влияет на результат по оценке модели (Random Forest).";
        case "dependency":
            return "Детальный анализ зависимости между двумя конкретными метриками. Помогает понять форму их взаимосвязи и визуально оценить плотность и разброс данных.";
        case "outliers":
            return "Диаграмма размаха (Boxplot) наглядно демонстрирует распределение данных: медиану, квартили и помогает быстро обнаружить статистические выбросы (аномалии).";
        case "category_count":
        case "numeric_hist":
            return "Гистограмма визуализирует частоту распределения данных. Показывает, какие значения (или категории) встречаются чаще всего, а какие — являются редкостью.";
        case "cash_flow_chart":
            return "График движения денежных средств (Cash Flow) показывает притоки и оттоки денег по периодам. Зеленые столбцы — это профицит, красные — дефицит (отток превышает приток). Помогает прогнозировать кассовые разрывы.";
        case "pnl_report":
            return "Отчет о прибылях и убытках (P&L). Отражает общую выручку, понесенные расходы и итоговую чистую прибыль (или убыток). Каскадная диаграмма (Waterfall) наглядно показывает, как доходы «съедаются» расходами.";
        case "expense_pie_chart":
            return "Структура расходов демонстрирует, на какие категории (статьи затрат) уходит основная часть бюджета компании.";
        case "abc_analysis":
            return "Классический закон Парето (80/20). Синие столбики — это ваша выручка по категориям. Красная линия показывает НАКОПЛЕННЫЙ процент. Как только красная линия пересекает отметку 80% — все товары слева от неё являются вашим ядром (группа А), которое приносит основные деньги.";
        case "unit_economics":
            return "Оценка эффективности каналов. Зеленый столбец (LTV) — выручка с клиента за всё время. Красный (CAC) — стоимость его привлечения. Оранжевая линия (ROMI) показывает окупаемость в процентах — ищите каналы с самым высоким ROMI.";
        case "revenue_forecast":
            return "Прогноз выручки на основе исторического тренда. Закрашенная зона (конус неопределенности) показывает возможный разброс доходов: от пессимистичного до оптимистичного сценария.";
        case "cohort_analysis":
            return 'Тепловая карта жизненного цикла клиентов (Retention). Показывает "здоровье" базы: строки — это когорты (пользователи, пришедшие в один месяц). Столбцы — месяцы с момента первой покупки. Темные ячейки означают высокую лояльность, белые пустоты — отток.';
        default:
            return "Визуализация данных для подробного анализа.";
    }
};
...
const getGranularityOptions = (
    ...
    if (!startDateStr || !endDateStr)
        return ["минуты", "часы", "дни", "недели", "месяца", "года"];
    try {
        ...
        if (diffMs > FIVE_YEARS) {
            return ["недели", "месяца", "года"];
        } else if (diffMs > YEAR) {
            return ["дни", "недели", "месяца", "года"];
        } else if (diffMs > SIX_MONTHS) {
            return ["дни", "недели", "месяца"];
        } else if (diffMs > MONTH) {
            return ["часы", "дни", "недели", "месяца"];
        } else if (diffMs > WEEK) {
            return ["минуты", "часы", "дни", "недели"];
        } else if (diffMs > DAY) {
            return ["секунды", "минуты", "часы", "дни"];
        } else if (diffMs > HOUR) {
            return ["секунды", "минуты", "часы"];
        } else {
            return ["секунды", "минуты"];
        }
    } catch (e) {
        return ["минуты", "часы", "дни", "недели", "месяца", "года"];
    }
...
const getFallbackGranularity = (options: string[], current: string): string => {
    if (options.includes(current)) return current;
    if (options.includes("дни")) return "дни";
    if (options.includes("минуты")) return "минуты";
    if (options.includes("недели")) return "недели";
    if (options.includes("месяца")) return "месяца";
    return options[0] || "дни";
};

// --- 3. ИНТЕРАКТИВНЫЙ КОМПОНЕНТ ДЛЯ ОДНОГО ГРАФИКА ---
const InteractiveChart: React.FC<{ chart: ChartData; preview?: boolean }> = ({
    chart,
    preview,
}) => {
        ...
        switch (chart.type) {
            case "correlation": {
                ctrls.push({
                    ...
                    id: "sign",
                    label: "Связь",
                    options: ["Все", "Только прямая (+)", "Только обратная (-)"],
                    defaultValue: "Все",
                });
                ctrls.push({
                    type: "slider",
                    id: "thresh_pos",
                    label: "Прямая корреляция >",
                    min: 0,
                    max: 1,
                    step: 0.05,
                    defaultValue: 0,
            ...
            case "trend_line": {
                ...
                if (xDates.length > 0) {
                    const minDate = xDates[0].split(/[T ]/)[0];
                    const maxDate = xDates[xDates.length - 1].split(/[T ]/)[0];
                    ctrls.push({
                        type: "date",
                        id: "date_start",
                        label: "От",
                ...
                ctrls.push({
                    ...
                    id: "granularity",
                    label: "Гранулярность",
                    options: ["минуты", "часы", "дни", "недели", "месяца", "года"],
                    defaultValue: "дни",
                });
                ctrls.push({
                    type: "multiselect",
                    id: "visible_columns",
                    label: "Показатели",
                    options: chart.data.numeric_cols || [],
                    defaultValue: (chart.data.numeric_cols || []).slice(0, 5),
                });
                break;
            }
            case "pairplot": {
                if (chart.data.all_columns?.length > 2) {
                    ctrls.push({
                        ...
                        id: "splom_columns",
                        label: "Признаки",
                        options: chart.data.all_columns,
                        defaultValue: chart.data.default_columns,
                    });
                }
                break;
            }
            case "feature_tree": {
                ...
                const maxDist =
                    dcoord.length > 0
                        ? Math.ceil(Math.max(...dcoord.flat()) * 10) / 10
                        : 2;
                ctrls.push({
                    type: "slider",
                    id: "cluster_threshold",
                    label: "Порог кластеров <",
                    min: 0,
                    max: maxDist,
                    step: 0.02,
                    defaultValue: 0.1,
    ...
    const [controlValues, setControlValues] = useState<Record<string, any>>(
        () => {
            ...
            // Dynamic adjustment for the initial granularity based on selected dates
            if (chart.type === "trend_line") {
                const start = init["date_start"];
                const end = init["date_end"];
                if (start && end) {
                    const allowed = getGranularityOptions(start, end);
                    if (!allowed.includes(init["granularity"])) {
                        init["granularity"] = getFallbackGranularity(
                            allowed,
                            init["granularity"],
                        );
                    }
    ...
    const handleControlChange = (id: string, val: any) => {
        setControlValues((prev) => {
            ...
            if (id === "date_start" || id === "date_end") {
                const start = next["date_start"];
                const end = next["date_end"];
                const allowed = getGranularityOptions(start, end);
                const currentGran = prev["granularity"] || "дни";
                if (!allowed.includes(currentGran)) {
                    next["granularity"] = getFallbackGranularity(allowed, currentGran);
                }
            }
            return next;
        });
    ...
    let chartTitle = chartDef
        ? chartDef.plotTitle(chart.data, controlValues)
        : "График";
    ...
    switch (chart.type) {
        case "correlation": {
            const { sign, thresh_pos, thresh_neg, hide_diag, precision } =
                controlValues;
            const cols = Object.keys(chart.data);
            const z_vals = cols.map((c1) =>
                cols.map((c2) => Number(chart.data[c1][c2])),
            );

            const z_filtered = z_vals.map((row, i) =>
                row.map((val, j) => {
            ...
            const text_filtered = z_filtered.map((row) =>
                row.map((val) => (val === null ? "" : val.toFixed(precision))),
            );
        ...
        case "trend_line": {
            ...
            if (startDate && endDate) {
                startIdx = chart.data.x.findIndex(
                    (d: string) => d.split(/[T ]/)[0] >= startDate,
                );
                if (startIdx === -1) startIdx = 0;
                for (let i = chart.data.x.length - 1; i >= 0; i--) {
                    if (chart.data.x[i].split(/[T ]/)[0] <= endDate) {
                        endIdx = i;
                        break;
                    }
                }
            ...
            const rawSlicedX = chart.data.x.slice(startIdx, sliceEnd);
            const rawSlicedY: Record<string, number[]> = {};
            numericCols.forEach((col: string) => {
                rawSlicedY[col] = (chart.data.y[col] || []).slice(startIdx, sliceEnd);
            });

            const granularity = controlValues["granularity"] || "дни";

            const groupedX: string[] = [];
            const groupedY: Record<string, number[]> = {};
            numericCols.forEach((col: string) => {
                groupedY[col] = [];
            });

            const pad = (n: number) => String(n).padStart(2, "0");
            const getGroupKey = (xVal: string, gran: string): string => {
                try {
                    // Try parsing the date component safely across multiple formats
                    const d = (() => {
                        let cleanVal = xVal;
                        if (xVal.includes(" ") && !xVal.includes("T")) {
                            cleanVal = xVal.replace(" ", "T");
                        }

                        let parsed = new Date(cleanVal);
                        if (!isNaN(parsed.getTime())) return parsed;

                        parsed = new Date(cleanVal.replace(/-/g, "/"));
                        if (!isNaN(parsed.getTime())) return parsed;

                        return parsed;
                    })();

                    if (isNaN(d.getTime())) return xVal;

                    // Extract components in UTC to avoid offset shifts
                    ...
                    switch (gran) {
                        case "секунды": {
                            ...
                            return `${year}-${pad(month + 1)}-${pad(date)} ${pad(hours)}:${pad(minutes)}:${pad(seconds)}`;
                        }
                        case "минуты":
                            return `${year}-${pad(month + 1)}-${pad(date)} ${pad(hours)}:${pad(minutes)}:00`;
                        case "часы":
                            return `${year}-${pad(month + 1)}-${pad(date)} ${pad(hours)}:00:00`;
                        case "недели": {
                            const temp = new Date(Date.UTC(year, month, date));
            ...
            const tempGroups: Record<
                string,
                { sum: Record<string, number>; count: number }
            > = {};
            const uniqueKeys: string[] = [];

            for (let i = 0; i < rawSlicedX.length; i++) {
                const xVal = rawSlicedX[i];
                const key = getGroupKey(xVal, granularity);
                if (!tempGroups[key]) {
                    tempGroups[key] = { sum: {}, count: 0 };
                    numericCols.forEach((col: string) => {
                        tempGroups[key].sum[col] = 0;
                    });
                    uniqueKeys.push(key);
                }
                tempGroups[key].count += 1;
                numericCols.forEach((col: string) => {
                    const val = Number(rawSlicedY[col]?.[i]) || 0;
                    tempGroups[key].sum[col] += val;
                });
            }

            // Sort unique keys chronologically
            uniqueKeys.sort();
            ...
            uniqueKeys.forEach((key) => {
                ...
                numericCols.forEach((col: string) => {
                    const avg = g.count > 0 ? g.sum[col] / g.count : 0;
                    groupedY[col].push(avg);
                });
            });
            ...
            plotLayout = preview
                ? previewLayout
                : {
                    datarevision: `${activeCols.join(",")}_${startDate}_${endDate}_${granularity}`,
                    xaxis: { automargin: true, type: "date" },
                    yaxis: { automargin: true, title: { text: "Значения метрик" } },
                    margin: { t: 20, b: 80, l: 80, r: 50 },
                    showlegend: true,
        ...
        case "pairplot": {
            ...
            const selectedList =
                (controlValues["splom_columns"] as string[]) ||
                chart.data.default_columns ||
                [];

            const activeOriginalLabels = allDims
                .filter((dim: any) =>
                    preview
                        ? chart.data.default_columns.includes(dim.label)
                        : selectedList.includes(dim.label),
                )
                .map((dim: any) => dim.label);

            const activeDims = allDims
                .filter((dim: any) => {
            ...
            for (let i = 1; i <= Math.max(5, activeDims.length); i++) {
                ...
                if (preview) {
                    splomLayout[`xaxis${suffix}`] = {
                        showticklabels: false,
                        visible: false,
                    };
                    splomLayout[`yaxis${suffix}`] = {
                        showticklabels: false,
                        visible: false,
                    };
                } else {
            ...
            if (!preview)
                chartTitle = `Матрица рассеяния (Выбрано признаков: ${activeDims.length})`;
            break;
        }
        ...
        case "dependency": {
            ...
            if (sub_type === "scatter") {
                ...
                plotLayout = preview
                    ? previewLayout
                    : {
                        margin: { t: 30, r: 50, b: 80, l: 120 },
                        xaxis: {
                            automargin: true,
                            title: { text: col2 },
                            tickangle: -45,
                        },
                        yaxis: { automargin: true, title: { text: col1 } },
                    };
            } else if (sub_type === "box") {
                plotData = chart.data.categories.map((cat: string, idx: number) => ({
                    y: chart.data.values[idx],
                    type: "box",
                    name: cat,
                    boxpoints: "outliers",
                    hoverinfo: preview ? "skip" : "all",
                ...
                plotLayout = preview
                    ? previewLayout
                    : {
                        xaxis: { automargin: true, title: { text: chart.data.cat_col } },
                        yaxis: {
                            automargin: true,
                            title: { text: chart.data.num_col },
                            zeroline: false,
                        },
                        margin: { t: 30, b: 80, l: 80, r: 50 },
                        showlegend: false,
        ...
        case "feature_tree": {
            ...
            links.forEach((parent: any) => {
                const px1 = parent.x[0],
                    py1 = parent.y[0];
                const px2 = parent.x[3],
                    py2 = parent.y[3];
                links.forEach((child: any) => {
                    if (parent.id === child.id) return;
                    const cx = (child.x[1] + child.x[2]) / 2,
                        cy = child.y[1];
                    if (Math.abs(px1 - cx) < 0.1 && Math.abs(py1 - cy) < 0.1)
                        child.parentId = parent.id;
                    else if (Math.abs(px2 - cx) < 0.1 && Math.abs(py2 - cy) < 0.1)
                        child.parentId = parent.id;
                });
            });
            ...
            sortedLinks.forEach((link: any) => {
                if (link.height > threshold) {
                    ...
                    const parentColor =
                        link.parentId !== null ? linkColorMap[link.parentId] : null;
                    ...
                    >
                                    ...
                                    {clusterEntries.map(([color, features], i) => (
                                        ...
                                        >
                                            ...
                                            <div style={{ fontSize: "12px", color: "#444" }}>
                                                <span style={{ fontWeight: 600 }}>Группа {i + 1}:</span>{" "}
                                                {features.join(", ")}
                                            </div>
                                        </div>
                                    ))}
                        ...
                        {isolated.length > 0 && (
                            ...
                            >
                                ...
                                {isolated.join(", ")} (их лучше не удалять)
                            </div>
                        )}
                    </div>
                );
            ...
            plotData = links.map((link: any) => ({
                ...
                type: "scatter",
                mode: "lines",
                line: { color: linkColorMap[link.id], width: 2 },
                hoverinfo: "none",
                showlegend: false,
            }));
        ...
        case "cash_flow_chart": {
            ...
            const colors = chart.data.values.map((v: number) =>
                v >= 0 ? "#3cb44b" : "#e6194b",
            );
        ...
        case "unit_economics": {
            plotData = [
                ...
                },
                {
                    ...
                    text:
                        chart.data.romi_text || chart.data.romi.map((r: any) => r + "%"),
        ...
        case "revenue_forecast": {
            plotData = [
                ...
                // 4. Средний прогноз (пунктир) — рисуем поверх заливки
                {
                    ...
                    line: { color: "#f58231", width: 3, dash: "dash" },
                },
            ];
            plotLayout = preview
                ? previewLayout
    ...
    const renderControlPanel = () => {
        ...
        return (
            <div
                style={{
                    background: "#f8f9fa",
                    padding: isFeatureTree ? "6px 12px" : "12px 16px",
                    borderRadius: "8px",
                    border: "1px solid #e1e4e8",
                    display: "flex",
                    gap: "20px",
                    alignItems: "center",
                    flexWrap: "wrap",
                    width: "100%", // Убрали marginBottom, так как это последний элемент шапки
                }}
            >
                {controls.map((ctrl) => {
                    ...
                    if (ctrl.type === "select") {
                        ...
                        if (ctrl.id === "granularity") {
                            const start = controlValues["date_start"];
                            const end = controlValues["date_end"];
                            options = getGranularityOptions(start, end);
                        }

                        const isOpen = openDropdown === ctrl.id;
                        const currentValue = controlValues[ctrl.id] || ctrl.defaultValue;
                                ...
                                <div style={{ position: "relative" }}>
                                    ...
                                    {isOpen && (
                                        ...
                                        >
                                            ...
                                            >
                                                {options.map((o) => {
                                                    const isSelected = o === currentValue;
                                                    return (
                                                        <div
                                                            key={o}
                                                            onClick={() => {
                                                                handleControlChange(ctrl.id, o);
                                                            ...
                                                            style={{
                                                                ...
                                                                fontSize: "12px",
                                                                cursor: "pointer",
                                                                color: isSelected ? "#328fec" : "#333333",
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <input
                                        ...
                                        value={val}
                                        onChange={(e) =>
                                            handleControlChange(ctrl.id, parseFloat(e.target.value))
                                        }
                                        style={{
                                            width: "100px",
                                            margin: 0,
                                        ...
                                        >
                                            <div
                                                ...
                                                style={{
                                                    ...
                                                    gap: "8px",
                                                    cursor: "pointer",
                                                    background: isAllSelected ? "#f0f7ff" : "transparent",
                                                    ...
                                                    <div
                                                        ...
                                                        onClick={(e) => {
                                                            ...
                                                            const next = isChecked
                                                                ? selectedList.filter((v: string) => v !== opt)
                                                                : [...selectedList, opt];
                                                            handleControlChange(ctrl.id, next);
                                                        }}
                                                        style={{
            ...
            {!preview && (
                ...
                <div
                    style={{
                        ...
                        boxSizing: "border-box",
                        textAlign: "left",
                        marginBottom: isFeatureTree ? "4px" : "12px",
                    }}
                >
                    <h3
                        style={{
                    ...
                    <div
                        style={{
                            fontSize: "13px",
                            color: "#555",
                            marginBottom: isFeatureTree ? "6px" : "12px",
                            lineHeight: "1.4",
                        }}
                    >
