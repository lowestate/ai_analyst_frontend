...
import { DataCharts } from "../chat/Charts";
...
const findFreeSpace = (
    ...
    while (hasOverlap && attempts < 50) {
        ...
        for (const item of items) {
            const overlapX =
                x < item.x + item.w + GRID_SIZE && x + w + GRID_SIZE > item.x;
            const overlapY =
                y < item.y + item.h + GRID_SIZE && y + h + GRID_SIZE > item.y;
...
export const Dashboard: React.FC = () => {
    ...
    >({});
    const [activeSettingsMenu, setActiveSettingsMenu] = useState<string | null>(
        null,
    );
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        if (chartData && folderId && canvasRef.current) {
            ...
            if (folderItems.length > 0) {
                ...
                const hitChart = folderItems.find(
                    (i) =>
                        newX >= i.x &&
                        newX <= i.x + i.w &&
                        newY >= i.y &&
                ...
                setGroupBounds((gb) => {
                    if (folderItems.length === 0) {
                        return {
                            ...
                            [folderId]: {
                                ...gb[folderId],
                                w: finalX + targetW + GRID_SIZE,
                                h: finalY + targetH + GRID_SIZE + 24,
                            },
                        };
                    } else {
    ...
    const updateItem = (
        ...
        setDashItems((prev) => {
            ...
            setGroupBounds((gb) => {
                const maxW = Math.max(
                    gb[folderId].w,
                    currentItem.x + currentItem.w + GRID_SIZE,
                );
                const maxH = Math.max(
                    gb[folderId].h,
                    currentItem.y + currentItem.h + GRID_SIZE + 24,
                );
                return { ...gb, [folderId]: { ...gb[folderId], w: maxW, h: maxH } };
            });
            return next;
